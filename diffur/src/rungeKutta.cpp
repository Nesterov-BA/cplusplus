...
void solutionUpToTimeFinal(double xStart, double yStart,
                           double f(double, double), double g(double, double),
                           double finish, double *cycleTimeLess,
                           double *cycleTimeMore, double *xEnd, double *yEnd) {
  ...
  while (time < finish) {
    if (time + 2 * step > finish) {
      ...
      Runge_Kutta4StepVariedSimple(xStart, yStart, f, g, &step, &tempX, &tempY,
                                   &errorSum, &globalError,
                                   &globalErrorRegular);
      numOfPoints++;
    }
    ...
    if (yStart * tempY < 0) {
      // printf("%.10f * %.10f, timeL = %f, timeR = %f, count = %d, errorSum =
    ...
    fileErr << globalError << " " << logNormCalc(xStart, yStart, tempX, tempY)
            << " " << time << endl;
    fileErrReg << globalErrorRegular << " "
               << regNormCalc(xStart, yStart, tempX, tempY) << " " << time
               << endl;
    xStart = tempX;
    yStart = tempY;
    x.push_back(xStart);
  ...
  *yEnd = yStart;
  ...
  // globalErrorReg: %e\n", tolerance, errorSum, numOfPoints, globalError,
  // globalErrorRegular);
  numberOfPoints << numOfPoints;
}

void solutionUpToTime(double xStart, double yStart, double f(double, double),
                      double g(double, double), double finish,
                      double *cycleTimeLess, double *cycleTimeMore,
...
void findCycle(double xStart, double yStart, double f(double, double),
               double g(double, double), double *xEnd, double *yEnd) {
  xStart = 1;
  yStart = 0;
  ...
  while (count < 100) {
    solutionUpToTime(xStartTemp, yStartTemp, f, g, 200, &cycleTimeLess,
                     &cycleTimeMore, &xEnd1, &yEnd1);
    chordStart = cycleTimeLess;
    chordEnd = cycleTimeMore;
    int count2 = 0;
    while (fabs(chordEnd - chordStart) > tolerance) {
      count2++;
      solutionUpToTime(xStartTemp, yStartTemp, f, g, chordStart, &cycleTimeLess,
                       &cycleTimeMore, &xEnd1, &yEnd1);
      solutionUpToTime(xStartTemp, yStartTemp, f, g, chordEnd, &cycleTimeLess,
                       &cycleTimeMore, &xEnd2, &yEnd2);
      chordTemp =
          chordStart - (chordEnd - chordStart) * yEnd1 / (yEnd2 - yEnd1);
      chordStart = chordEnd;
      chordEnd = chordTemp;
    }
    ...
    if (fabs(xStartTemp - xEnd2) < tolerance * 100) {
      ...
      *yEnd = yEnd2;
      ...
      solutionUpToTimeFinal(xStartTemp, yStartTemp, f, g, chordStart,
                            &cycleTimeLess, &cycleTimeMore, &xEnd1, &yEnd1);
...
void Runge_Kutta4ClassicSimple(double startX, double startY, double step,
                               double f(double, double),
                               double g(double, double), double *endX,
                               double *endY) {
...
void Runge_Kutta4StepVariedSimple(double startX, double startY,
                                  double f(double, double),
                                  double g(double, double), double *step,
                                  double *endX, double *endY, double *errorSum,
                                  double *globalError,
                                  double *globalErrorRegular) {
  double err = 1;
  ...
  while (err > tolerance) {
    ...
    err = fabs((*endX - tempEndX) / dX) > fabs((*endY - tempEndY) / dY)
              ? fabs((*endX - tempEndX) / dX)
              : fabs((*endY - tempEndY) / dY);
    err /= (16 - 1);
...
void checkCycle(double f(double, double), double g(double, double)) {
  ...
  solutionUpToTime(xStart, yStart, f, g, 50, &cycleTimeLess, &cycleTimeMore,
                   &xEnd, &yEnd);
  // printf("\ncycle1: %.10f, %.10f\n", cycleTimeLess, cycleTimeMore);
  double tStart = cycleTimeLess;
  double tEnd = cycleTimeMore;
  solutionUpToTime(xStart, yStart, f, g, tStart, &cycleTimeLess, &cycleTimeMore,
                   &xEnd1, &yEnd1);
  solutionUpToTime(xStart, yStart, f, g, tEnd, &cycleTimeLess, &cycleTimeMore,
                   &xEnd2, &yEnd2);
  // printf("ends: %.10f, %.10f\n", yEnd1, yEnd2);
}
